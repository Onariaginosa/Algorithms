		...
		if (state.isWin()) {
			...
			for (T3Action action : children.keySet()) {
				if (children.get(action).isWin()) {
					...
					data.utilityCost = Math.max(data.utilityCost, alphabeta(children.get(action), alpha, beta, false).utilityCost);
			...
			for (T3Action action : children.keySet()) {
				data.utilityCost = Math.min(data.utilityCost, alphabeta(children.get(action), alpha, beta, true).utilityCost);
