 ...
 *  Purpose       :  Creates the state object that preserves each state of the game
 *  @author       :  Andrew Forney
 *  @author       :  Ona Igbinedion (get Transitions method)
...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
    	...
    	T3Action currentAction = null;
    	for (int c = 0; c <=2; c++) {
    		for(int r = 0; r<=2; r++) {
    			for(int move : getMoves()) {
    				currentAction = new T3Action(c,r,move);
    				if (isValidAction(currentAction)) {
    					transitions.put(currentAction,getNextState(currentAction));
    				}
    			}
