...
public class Huffman {
	...
	/*
	 * Creates a character distribution for the corpus
	 *  @param corpus A String representing a message / document corpus with
	 *               distributions over characters that are implicitly used
	 *               throughout the methods that follow. Note: this corpus ONLY
	 *               establishes the Encoding Map; later compressed corpi may
	 *               differ.
	 * @return A treeMap representation of the character distribution
	 */
	...
	/*
	 * Builds the huffman trie from the character distribution
	 * @param charDist  the character distribution of the given corpus
	 */
	...
	/*
	 * Creates the Encoding Map from the huffman trie
	 * @param current  the current huffman trie node you are on
	 * @param collected the encoding map value to be added
	 */
	private void createEncodingMap(HuffNode current, String collected) {
		if (current.isLeaf()) {
			encodingMap.put(current.character, collected);
			if(this.longestEncodingLength < collected.length()) {
